...
CREATE EXTERNAL TABLE catalogue (ProductColorId INT, 
                      GenderLabel STRING,
...
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/catalogue/"
TBLPROPERTIES ("skip.header.line.count"="1");

-- Creation of the table customer
DROP TABLE IF EXISTS customer;
CREATE EXTERNAL TABLE customer (CustomerId INT,
                       DomainCode STRING,
                       BirthDate DATE,
                       Gender STRING,
                       Size int)
...
FIELDS TERMINATED BY '\073' 
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/customer/"
TBLPROPERTIES ("skip.header.line.count"="1");

-- Creation of the table order
DROP TABLE IF EXISTS order1;
CREATE EXTERNAL TABLE order1 (OrderNumber int,
                    VariantId int, 
                    CustomerId int,
                    Quantity int,
                    UnitPrice int,
...
FIELDS TERMINATED BY '\073' 
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/order1/"
TBLPROPERTIES ("skip.header.line.count"="1");

-- Creation of the table reference

DROP TABLE IF EXISTS reference;
CREATE EXTERNAL TABLE reference (VariantId int,
                    ProductColorId int,
                    ProductId int)
ROW FORMAT DELIMITED 
FIELDS TERMINATED BY '\073' 
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/reference/"
TBLPROPERTIES ("skip.header.line.count"="1");

-- Create the data variant

DROP TABLE IF EXISTS variant;
CREATE EXTERNAL TABLE variant (VariantId int,
                      MinSize int,	MaxSize int,
                      Size String)
ROW FORMAT DELIMITED 
FIELDS TERMINATED BY '\073' 
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/variant/"
TBLPROPERTIES ("skip.header.line.count"="1");
...
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/result_product/"
 AS
...
DROP TABLE IF EXISTS Customer_Aggregation;
CREATE TABLE IF NOT EXISTS Customer_Aggregation
ROW FORMAT DELIMITED 
FIELDS TERMINATED BY ',' 
STORED AS TEXTFILE
LOCATION "/user/hue/data_tool/result_customer/"
 AS
